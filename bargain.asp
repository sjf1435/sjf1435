...
Dim rsb__mmnum
rsb__mmnum = "%"
If (request.form("b_num") <> "") Then 
  rsb__mmnum = request.form("b_num")
End If
%>
<%
Dim rsb__mmcompany
rsb__mmcompany = "%"
If (request.form("b_company") <> "") Then 
  rsb__mmcompany = request.form("b_company")
End If
%>
<%
Dim rsb__mmsign
rsb__mmsign = "%"
If (request.form("b_sign") <> "") Then 
  rsb__mmsign = request.form("b_sign")
End If
%>
<%
Dim rsb__mmmoney
rsb__mmmoney = "0"
If (request.form("b_money")  <> "") Then 
  rsb__mmmoney = request.form("b_money") 
End If
%>
<%
Dim rsb
...
if ((rsb__mmnum <> "%") or (rsb__mmcompany <> "%") or (cint(rsb__mmmoney) <> 0) or (rsb__mmsign <> "%")) then
  if request.form("select") = "大于" then
  rsb.Source = "SELECT *  FROM t_bargain  WHERE b_num like '%" + Replace(rsb__mmnum, "'", "''") + "%' and b_company like '%" + Replace(rsb__mmcompany, "'", "''") + "%' and b_money > " + Replace(rsb__mmmoney, "'", "''") + " and b_sign like '%" + Replace(rsb__mmsign, "'", "''") + "%'  ORDER BY b_id desc"
  end if
  if request.form("select") = "小于" then
  rsb.Source = "SELECT *  FROM t_bargain  WHERE b_num like '%" + Replace(rsb__mmnum, "'", "''") + "%' and b_company like '%" + Replace(rsb__mmcompany, "'", "''") + "%' and b_money < " + Replace(rsb__mmmoney, "'", "''") + " and b_sign like '%" + Replace(rsb__mmsign, "'", "''") + "%'  ORDER BY b_id desc"
  end if
  if request.form("select") = "等于" then
  rsb.Source = "SELECT *  FROM t_bargain  WHERE b_num like '%" + Replace(rsb__mmnum, "'", "''") + "%' and b_company like '%" + Replace(rsb__mmcompany, "'", "''") + "%' and b_money = " + Replace(rsb__mmmoney, "'", "''") + " and b_sign like '%" + Replace(rsb__mmsign, "'", "''") + "%'  ORDER BY b_id desc"
  end if
else
...
<h1>合同管理 </h1>
<hr size="1">
<form name="form1" method="post" action="bargain.asp">
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td>合同号:
        <input name="b_num" type="text" id="b_num">
        发展商:
        <input name="b_company" type="text" id="b_company">
        总金额
        <select name="select" class="px12">
          <option value="大于" selected>大于</option>
          <option value="小于">小于</option>
          <option value="等于">等于</option>
        </select>
        <input name="b_money" type="text" id="b_money2" size="10">
        是否签约 
        <select name="b_sign" id="b_sign">
...
<hr size="1">
<br>
记录 <%=(rsb_first)%> 到 <%=(rsb_last)%> (总共 <%=(rsb_total)%> 个记录) <br>
<br>
<table width="100%" border="0" align="center" bordercolor="#999999" bgcolor="#FFFFFF" class="border-all">
  ...
  <tr align="center"> 
    ...
    <td> 
      <% if (rsb.Fields.Item("b_check").Value) <> "审核" then %>
      <a href="b_check.asp?b_id=<%=(rsb.Fields.Item("b_id").Value)%>">审核</a>
      <% end if %>
      <a href="b_updata.asp?b_id=<%=(rsb.Fields.Item("b_id").Value)%>">修改</a> 
      <a href="b_del.asp?b_id=<%=(rsb.Fields.Item("b_id").Value)%>" onClick="javascript:return confirm('请确认要删除该合同 ')">删除</a> 
    </td>
  </tr>
  <% 
